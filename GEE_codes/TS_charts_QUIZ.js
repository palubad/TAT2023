...
Note: This is the "QUIZ" code. QUIZ is not a variable, it is a placeholder, where
you should add your code. Every time you have a comment next to a QUIZ, what should 
you do. You can get inspiration from the "solution" code.
...
var S1Collection = ee.ImageCollection('COPERNICUS/S1_GRD_FLOAT')
                  .filterBounds(selected) // 'Filter by area of interest'
                  .filterDate(startDate, endDate) // 'Filter dates'
...
var S2 = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
          .QUIZ // 'Add a function to filter by area of interest'
          .QUIZ // 'Add a function to filter dates'
          .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE',max_clouds));

// Check out the size of our S2 image collection
// QUIZ // 'Print the size of S2 image collection'
...
var addOpticalVI = function(img) {
  ...
  var NDVI = img.normalizedDifference(['B8', 'B4']).rename('NDVI'), 
      ...
      NDWI = QUIZ // Add Normalized Difference Wetness Index
      NDMI = QUIZ // Add Normalized Difference Moisture Index
      NBR = QUIZ // Add Normalized Burn Ratio
...
S1Collection = S1Collection.map(QUIZ).select(QUIZ);
